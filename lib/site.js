...
const pillars = [
  ...
  { slug:'mcp-architecture', title:'MCP Architecture', description:'The host-client-server architecture of the Model Context Protocol, covering how hosts manage clients, how clients connect to servers, and how tools, resources, and prompts are exchanged.' },
  { slug:'mcp-transports', title:'MCP Transports', description:'The communication surfaces available for MCP clients and servers, including stdio for local processes and Streamable HTTP for remote connections.' },
  { slug:'mcp-governance', title:'MCP Governance', description:'How the Model Context Protocol is governed, versioned, and evolved through community contribution and specification maintenance.' }
];
const topics = [
  ...
  { slug:'http', title:'HTTP for MCP', description:'How MCP servers communicate over HTTP for request-response interactions, including Streamable HTTP transport patterns.' },
  { slug:'stdio-stderr', title:'stdio-stderr Transport', description:'How local MCP servers communicate using standard input, output, and error streams, including stderr separation for logs and diagnostics.' },
  { slug:'auth', title:'MCP Authorization', description:'How MCP implementations handle authorization and authentication when clients connect to servers or access protected resources and tools.' },
  { slug:'sampling', title:'MCP Sampling', description:'How MCP servers request text completions from the client-side LLM through the sampling capability, including client approval boundaries.' }
];
const glossary = [
  ...
  { slug:'json-rpc', title:'JSON-RPC', description:'A remote procedure call protocol encoded in JSON. MCP uses JSON-RPC 2.0 over its supported transports for structured request and response messaging.' },
  { slug:'transport', title:'Transport', description:'The mechanism by which MCP clients and servers communicate (for example stdio, Streamable HTTP, or WebSocket).' },
  { slug:'resource', title:'Resource', description:'A piece of data or content that an MCP server can expose to clients through resource URIs.' },
  { slug:'prompt', title:'Prompt', description:'A predefined template or set of instructions that an MCP server can offer to clients for structured interactions.' },
  { slug:'tool', title:'Tool', description:'A callable capability exposed by an MCP server, described with an input schema, that clients can invoke through the protocol.' },
  { slug:'sampling', title:'Sampling', description:'An MCP capability allowing servers to request LLM-generated completions from the client, subject to client-side configuration and human approval.' }
];
const comparisons = [
  ...
  { slug:'mcp-vs-graphql', title:'MCP vs GraphQL', description:'MCP standardizes callable tool discovery and execution; GraphQL standardizes typed data querying and mutation through a single endpoint.' },
  { slug:'mcp-vs-openapi', title:'MCP vs OpenAPI', description:'MCP describes dynamic, executable server capabilities at runtime; OpenAPI describes static HTTP endpoints and request/response shapes.' },
  { slug:'mcp-vs-grpc', title:'MCP vs gRPC', description:'MCP uses JSON-RPC over multiple transports with an emphasis on AI-agent integration; gRPC uses HTTP/2 with protobuf for strongly typed service contracts.' },
  { slug:'mcp-vs-webhooks', title:'MCP vs Webhooks', description:'MCP supports bidirectional client-server communication for tool execution; webhooks are server-to-server push notifications for discrete events.' }
];
...
function resolve(path){
  ...
  if((m=path.match(/^\/glossary$/))){ return {path,title:'MCP Glossary',description:'Definitions of key Model Context Protocol terms.',kind:'glossaryList',item:{},indexable:true}; }
  if((m=path.match(/^\/topics$/))){ return {path,title:'MCP Topics',description:'Concepts and transports in the MCP ecosystem.',kind:'topicList',item:{},indexable:true}; }
  if((m=path.match(/^\/pillars$/))){ return {path,title:'MCP Pillars',description:'Foundational concepts of the Model Context Protocol.',kind:'pillarList',item:{},indexable:true}; }
  if((m=path.match(/^\/compare$/))){ return {path,title:'MCP Comparisons',description:'How MCP compares to other protocols and technologies.',kind:'comparisonList',item:{},indexable:true}; }
  return null;
}
...
function related(current=''){
  const links=[['/directory','Server directory'],['/verification-methodology','Verification methodology'],['/pillars/mcp-registry','Official Registry guide'],['/what-is-mcp','What is MCP?'],['/security','MCP security'],['/topics/streamable-http','Streamable HTTP'],['/topics/stdio-transport','stdio transport'],['/glossary/mcp-server','MCP server glossary'],['/clients','MCP clients'],['/learn','Learning hub']];
...
const FOOT=`<footer class="footer"><div class="wrap"><strong>Explore MCPserver.in</strong><div class="footer-links">${link('/integrations','Integrations')}${link('/docs','Docs')}${link('/tools/mcp-playground','Playground')}${link('/state-of-mcp','State of MCP')}${link('/blog','Blog')}${link('/pricing','Availability')}${link('/about','About')}${link('/research/mcp-directories','Directory research')}${link('/hosting','Hosting')}${link('/editorial-policy','Editorial policy')}${link('/pillars/mcp-security','Security pillar')}${link('/glossary/mcp-client','MCP client glossary')}${link('/glossary/mcp-host','MCP host glossary')}${link('/compare/mcp-vs-rest','MCP vs REST')}${link('/glossary','Glossary')}${link('/topics','Topics')}${link('/pillars','Pillars')}${link('/compare','Comparisons')}</div><div>Independent MCP directory and developer knowledge platform. Registry snapshot: ${SNAPSHOT_DATE}. ${link('/privacy','Privacy')} · ${link('/terms','Terms')} · ${link('/contact','Contact')}</div></div></footer>`;
...
function genericBody(r){
  ...
  if(r.kind==='glossary'){ const item=r.item; return `<section class="hero"><p class="kicker">Glossary</p><h1>${esc(r.title)}</h1><p class="lead">${esc(item.description)}</p></section><section class="section"><div class="panel"><h2>Definition</h2><p>${esc(item.description)}</p></div></section>${relatedHtml(r.path)}`; }
  if(r.kind==='topic'){ const item=r.item; return `<section class="hero"><p class="kicker">Topic</p><h1>${esc(r.title)}</h1><p class="lead">${esc(item.description)}</p></section><section class="section"><div class="panel"><h2>Explanation</h2><p>${esc(item.description)}</p></div></section>${relatedHtml(r.path)}`; }
  if(r.kind==='pillar'){ const item=r.item; return `<section class="hero"><p class="kicker">Pillar</p><h1>${esc(r.title)}</h1><p class="lead">${esc(item.description)}</p></section><section class="section"><div class="panel"><h2>Overview</h2><p>${esc(item.description)}</p></div></section>${relatedHtml(r.path)}`; }
  if(r.kind==='compare'){ const item=r.item; return `<section class="hero"><p class="kicker">Comparison</p><h1>${esc(r.title)}</h1><p class="lead">${esc(item.description)}</p></section><section class="section"><div class="panel"><h2>Comparison</h2><p>${esc(item.description)}</p></div></section>${relatedHtml(r.path)}`; }
  if(r.kind==='glossaryList'){ const items=glossary.map(g=>`<article class="card"><h3>${esc(g.title)}</h3><p>${esc(g.description)}</p>${link('/glossary/'+g.slug,'View glossary entry →')}</article>`); return `<section class="hero"><p class="kicker">Glossary</p><h1>MCP Glossary</h1><p class="lead">Browse definitions of key MCP terms.</p></section><section class="section"><div class="grid">${items.join('')}</div></section>${relatedHtml(r.path)}`; }
  if(r.kind==='topicList'){ const items=topics.map(t=>`<article class="card"><h3>${esc(t.title)}</h3><p>${esc(t.description)}</p>${link('/topics/'+t.slug,'View topic →')}</article>`); return `<section class="hero"><p class="kicker">Topics</p><h1>MCP Topics</h1><p class="lead">Explore key concepts and transports in the MCP ecosystem.</p></section><section class="section"><div class="grid">${items.join('')}</div></section>${relatedHtml(r.path)}`; }
  if(r.kind==='pillarList'){ const items=pillars.map(p=>`<article class="card"><h3>${esc(p.title)}</h3><p>${esc(p.description)}</p>${link('/pillars/'+p.slug,'View pillar →')}</article>`); return `<section class="hero"><p class="kicker">Pillars</p><h1>MCP Pillars</h1><p class="lead">Explore foundational concepts of the Model Context Protocol.</p></section><section class="section"><div class="grid">${items.join('')}</div></section>${relatedHtml(r.path)}`; }
  if(r.kind==='comparisonList'){ const items=comparisons.map(c=>`<article class="card"><h3>${esc(c.title)}</h3><p>${esc(c.description)}</p>${link('/compare/'+c.slug,'View comparison →')}</article>`); return `<section class="hero"><p class="kicker">Comparisons</p><h1>MCP Comparisons</h1><p class="lead">See how MCP compares to other protocols and technologies.</p></section><section class="section"><div class="grid">${items.join('')}</div></section>${relatedHtml(r.path)}`; }
...
export function allIndexablePaths(){ 
  const paths = [
    ...
    ...['/glossary','/topics','/pillars','/compare'],
  ];
