...
function resolve(path){
  ...
  let m;
  if((m=path.match(/^\/servers\/([^/]+)$/))){ const item=serverRecords.find(s=>s.slug===m[1]); if(item) return {path,title:`${item.title} MCP Server`,description:item.description,kind:'server',item,indexable:true}; }
...
function schemaFor(r){
  ...
  if(r.path==='/') graph.push({'@type':'ItemList','@id':ORIGIN+'/#featured-servers',itemListElement:serverRecords.slice(0,8).map((s,i)=>({'@type':'ListItem',position:i+1,url:canonicalFor('/servers/'+s.slug),name:s.title}))});
...
const FOOT=`<footer class="footer"><div class="wrap">Independent MCP directory and developer knowledge platform. Registry snapshot: ${SNAPSHOT_DATE}. ${link('/privacy','Privacy')} · ${link('/terms','Terms')} · ${link('/contact','Contact')}</div></footer>`;
...
function directoryBody(){ return `<section class="hero"><p class="kicker">Registry-backed catalog</p><h1>MCP Server Directory</h1><p class="lead">This production surface contains ${serverRecords.length} quality-gated records from the Official MCP Registry snapshot dated ${SNAPSHOT_DATE}. The displayed count is derived from the dataset, not hand-authored marketing copy.</p></section><section class="section"><h2>Browse verified records</h2>${cards(serverRecords)}</section><section class="section"><h2>Categories derived from the same dataset</h2><div class="grid">${categoryRecords.map(c=>`<article class="card"><h3>${esc(c.name)}</h3><p>${c.count} record${c.count===1?'':'s'} in this snapshot.</p>${link('/categories/'+c.slug,'Open category →')}</article>`).join('')}</div></section>${relatedHtml('/directory')}`; }
function genericBody(r){
  if(r.kind==='directory') return directoryBody();
  if(r.kind==='server') return serverBody(r);
  if(r.kind==='category'){ const matches=serverRecords.filter(s=>s.category===r.item.name); return `<section class="hero"><p class="kicker">Dataset taxonomy</p><h1>${esc(r.title)}</h1><p class="lead">${esc(r.description)} This category contains ${matches.length} record${matches.length===1?'':'s'} in the ${SNAPSHOT_DATE} snapshot.</p></section><section class="section">${cards(matches)}</section>${relatedHtml(r.path)}`; }
  ...
  if(r.kind==='home') return `<section class="hero"><p class="kicker">Evidence-backed MCP catalog</p><h1>MCP server discovery without invented metrics.</h1><p class="lead">Browse ${serverRecords.length} quality-gated records from the Official MCP Registry snapshot used by this production build. Unknown values stay unknown; commercial, certification, latency, and uptime claims are not invented.</p><p>${link('/directory','Browse the directory →')}</p></section><section class="section"><h2>Featured verified servers</h2>${cards(serverRecords.slice(0,6))}</section><section class="section"><div class="notice">Source of truth: <a href="${REGISTRY_SOURCE}">Official MCP Registry API</a>. The Official Registry is currently a preview service, so records are snapshot-dated and can change.</div></section>${relatedHtml('/')}`;
...
export function renderRegistryJson(){ return JSON.stringify({ source:REGISTRY_SOURCE, snapshotDate:SNAPSHOT_DATE, count:serverRecords.length, servers:serverRecords.map(publicServerRecord) },null,2); }
export function renderServersJson(){ return JSON.stringify(serverRecords.map(publicServerRecord),null,2); }

export function allIndexablePaths(){ return [
  ...STATIC.map(r=>r.path),
  ...serverRecords.map(s=>'/servers/'+s.slug),
...
export function renderLlms(full=false){ const lines=[`# MCPserver.in`,`Canonical: ${ORIGIN}/`,`Registry source: ${REGISTRY_SOURCE}`,`Registry snapshot: ${SNAPSHOT_DATE}`,`Published catalog records: ${serverRecords.length}`,'','## Key pages','- /directory — registry-backed server directory','- /verification-methodology — evidence rules','- /security — security guidance and certification boundaries']; if(full){ lines.push('','## Servers',...serverRecords.map(s=>`- /servers/${s.slug} — ${s.title} ${s.latestVerifiedVersion}`)); } return lines.join('\n')+'\n'; }
